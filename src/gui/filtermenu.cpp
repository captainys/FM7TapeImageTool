...
class FsGuiMainCanvas::ForceSineWaveDialog : public FsGuiDialog
{
public:
	FsGuiButton *okBtn,*cancelBtn,*withinSelectedRegionBtn;
	FsGuiButton *dontCareBtn,*lowFirstBtn,*highFirstBtn;
	FsGuiTextBox *srcMinLenTxt,*srcMaxLenTxt;
	void Make(void);
	void OnButtonClick(FsGuiButton *btn) override;
};

void FsGuiMainCanvas::ForceSineWaveDialog::Make(void)
{
	okBtn=AddTextButton(0,FSKEY_NULL,FSGUI_PUSHBUTTON,"OK",YSTRUE);
	cancelBtn=AddTextButton(0,FSKEY_NULL,FSGUI_PUSHBUTTON,"Cancel",YSTRUE);
	withinSelectedRegionBtn=AddTextButton(0,FSKEY_NULL,FSGUI_CHECKBOX,"Within Selected Region",YSTRUE);
...
void FsGuiMainCanvas::ForceSineWaveDialog::OnButtonClick(FsGuiButton *btn)
{
	if(okBtn==btn)
	{
		CloseModalDialog(1);
...
void FsGuiMainCanvas::Filter_ForceSineWave(FsGuiPopUpMenuItem *)
{
	auto dlg=FsGuiDialog::CreateSelfDestructiveDialog<ForceSineWaveDialog>();
	dlg->Make();
	AttachModalDialog(dlg);
	ArrangeDialog();
	...
	dlg->CallOnCloseModal([=](int returnCode)
		{
			if(0!=returnCode)
			{
				...
				YsString cmd;
				cmd.Printf("EDIT FORCESINEWAVE %d %d %lld %lld %d %d",
				   channel,
				   sourceWaveType,
				   i0,
				   i1,
